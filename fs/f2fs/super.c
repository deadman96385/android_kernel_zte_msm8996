...
F2FS_RW_ATTR(F2FS_SBI, f2fs_sb_info, reserved_clusters, s_resv_clusters);
...
static int f2fs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	buf->f_bavail = user_block_count - valid_user_blocks(sbi) - sbi->s_resv_clusters;
	if (user_block_count - valid_user_blocks(sbi) < sbi->s_resv_clusters) {
		buf->f_bavail = 0;
	}
