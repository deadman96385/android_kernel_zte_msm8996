...
{
	s64 resv_clusters = sbi->s_resv_clusters;
	...
	if (unlikely(((sbi->total_valid_block_count + *count) > (sbi->user_block_count - resv_clusters))
		     || (sbi->user_block_count < resv_clusters))) {
						...
						struct inode *inode)
{
	...
	if (unlikely(valid_block_count > (sbi->user_block_count - resv_clusters))) {
		spin_unlock(&sbi->stat_lock);
		return false;
	}
