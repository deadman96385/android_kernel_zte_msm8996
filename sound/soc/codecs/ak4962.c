...
int g_hs_state = 0;
static int hs_status_read(struct seq_file *m, void *v)
{
	pr_info("%s: hs status %d\n", __func__, g_hs_state);
...
{
	return single_open(file, hs_status_read, NULL);
}
...
static irqreturn_t ak4962_jde_irq(int irq, void *data)
{
	...
	if (val & 0x01) {
		...
		/* lhs for test */
#ifdef P996A22_HARDVER_DIV
		board_id = check_hw_id();
		if (board_id == 1)
			snd_soc_update_bits(codec, IMPEDANCE_DETECTION, 0x1F, 0x02);
		else if (board_id == 2)
			snd_soc_update_bits(codec, IMPEDANCE_DETECTION, 0x1F, 0x00);
		else if (board_id == 3)
			snd_soc_update_bits(codec, IMPEDANCE_DETECTION, 0x1F, 0x00);
