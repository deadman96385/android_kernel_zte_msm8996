...
#define M1120_DATA_TOLERANCE                (0)
#define M1120_DATA_REDUNDANCY               (6)
#define M1120_EVENT_TYPE					EV_REL	/*EV_KEY*/
#define M1120_EVENT_CODE					REL_X	/*KEY_F1*/
...
static irqreturn_t m1120_irq_hallswitch_handler(int irq, void *dev_id);
...
static int	m1120_get_mode(struct device *dev);
...
static int	m1120_get_result_status_for_PV(m1120_data_t *p_data, int raw);
static int m1120_power_ctl(m1120_data_t *data, bool on);
static int m1120_get_current_status(struct device *dev, struct m1120_raw_data *p_raw_data);
/* ********************************************************* */
...
static irqreturn_t m1120_irq_handler(int irq, void *dev_id)
{
	...
	if (p_m1120_data != NULL) {
		dbg("run schedule_delayed_work");
		wake_lock_timeout(&p_m1120_data->wake_lock, msecs_to_jiffies(M1120_TIMEOUT_WAKELOCK));
...
static void m1120_work_hallswitch_func(struct work_struct *work_hallswitch)
{
	...
	pr_info("m1120_work_hallswitch_func: hall switch state is %d\n", p_data->hallswitchstate);
...
/* ********************************************************* */
/* functions for hallswitch interrupt handler */
/* ********************************************************* */
static irqreturn_t m1120_irq_hallswitch_handler(int irq, void *dev_id)
{
	dbg_func_in();
	if (p_m1120_data != NULL) {
		dbg("run schedule_delayed_work");
		schedule_delayed_work(&p_m1120_data->work_hallswitch, 0);
	}
	pr_info("m1120_irq_hallswitch_handler : the INTB has interrupt single now\n");
	return IRQ_HANDLED;
}
/* ********************************************************* */
...
static void m1120_set_init_state(struct device *dev)
{
	...
	err = m1120_measure_drdy(p_data, &initraw);
	pr_info("m1120_set_init_state : rawData--== %6d\n", initraw);
	if (!err) {
		p_data->last_data = m1120_get_result_status(p_data, initraw);
		if (p_data->last_data == M1120_RESULT_STATUS_B) {
			p_data->hallswitchstate = gpio_get_value(p_data->gpio_hallswitch);
			pr_info("m1120_set_init_state : hallswitchstate %d\n", p_data->hallswitchstate);
			if (p_data->hallswitchstate == 0) {
				p_data->last_data = M1120_RESULT_STATUS_A;
			}
			input_report_rel(p_data->input_dev, M1120_EVENT_CODE, p_data->last_data);
...
static void m1120_set_enable(struct device *dev, int enable)
{
	...
	if (enable) { /* enable if state will be changed */
		...
		if (atomic_cmpxchg(&p_data->atm.enable, 1, 0)) {
			...
			cancel_delayed_work_sync(&p_data->work_hallswitch);
...
/* 0x00 --> polling mode,  0x80 --> interrupt mode */
static int m1120_get_mode(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	m1120_data_t *p_data = i2c_get_clientdata(client);

	return atomic_read(&p_data->atm.mode);
}
...
static int m1120_update_interrupt_threshold(struct device *dev, short raw)
{
	...
	if (p_data->reg.map.intsrs & M1120_DETECTION_MODE_INTERRUPT) {
		...
		if (p_data->reg.map.intsrs & M1120_VAL_INTSRS_INTTYPE_WITHIN) {
			m1120_convdata_short_to_2byte(p_data->reg.map.opf, p_data->thrhigh - M1120_DATA_REDUNDANCY, &hthh, &hthl);
			m1120_convdata_short_to_2byte(p_data->reg.map.opf, p_data->thrlow + M1120_DATA_REDUNDANCY, &lthh, &lthl);
		} else {
			m1120_convdata_short_to_2byte(p_data->reg.map.opf, p_data->thrhigh + M1120_DATA_REDUNDANCY, &hthh, &hthl);
			m1120_convdata_short_to_2byte(p_data->reg.map.opf, p_data->thrlow - M1120_DATA_REDUNDANCY, &lthh, &lthl);
		}
...
static int m1120_set_operation_mode(struct device *dev, int mode)
{
	...
	case OPERATION_MODE_POWERDOWN:
		if (p_data->irq_enabled) {
			/* disable irq */
			err = disable_irq_wake(p_data->irq);
			if (err) {
				mxerr(dev, "disable irq wake was failed");
			}
			disable_irq(p_data->irq);
			free_irq(p_data->irq, NULL);
			err = disable_irq_wake(p_data->irq_hallswitch);
			if (err) {
				mxerr(dev, "disable irq_hallswitch wake was failed");
			}
			disable_irq(p_data->irq_hallswitch);
			free_irq(p_data->irq_hallswitch, NULL);
	...
	case OPERATION_MODE_MEASUREMENT:
		...
		if (p_data->reg.map.intsrs & M1120_DETECTION_MODE_INTERRUPT) {
			if (!p_data->irq_enabled) {
				...
				err = enable_irq_wake(p_data->irq);
				if (err) {
					mxerr(dev, "enable irq wake was failed");
					disable_irq(p_data->irq);
					return err;
				}

				err = request_irq(p_data->irq_hallswitch, &m1120_irq_hallswitch_handler,
						IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT,
						M1120_IRQ_HALLSWITCH_NAME, 0);
				if (err) {
					mxerr(dev, "request_irq_hallswitch was failed");
					return err;
				}
				disable_irq(p_data->irq_hallswitch);
				mxinfo(dev, "request_irq_hallswitch was success");
				enable_irq(p_data->irq_hallswitch);
				err = enable_irq_wake(p_data->irq_hallswitch);
				if (err) {
					mxerr(dev, "enable irq_hallswitch wake was failed");
					disable_irq(p_data->irq_hallswitch);
					return err;
				}
...
static int m1120_set_detection_mode(struct device *dev, u8 mode)
{
	...
	if (mode & M1120_DETECTION_MODE_INTERRUPT) {
		...
		atomic_set(&p_data->atm.mode, M1120_DETECTION_MODE_INTERRUPT);
	} else {
		/* write intsrs */
		...
		atomic_set(&p_data->atm.mode, M1120_DETECTION_MODE_POLLING);
	}
...
static int m1120_get_result_status(m1120_data_t *p_data, int raw)
{
	...
	case M1120_RESULT_STATUS_A:
		pr_info("Result is status [A 0]\n");
		break;
	case M1120_RESULT_STATUS_B:
		pr_info("Result is status [B (0,180)]\n");
		break;
	case M1120_RESULT_STATUS_C:
		pr_info("Result is status [C 180]\n");
		break;
	default:
		break;
	}
...
static int m1120_get_result_status_for_PV(m1120_data_t *p_data, int raw)
{
	...
	if (p_data->thrlow - M1120_DATA_REDUNDANCY >= raw) {
		status = M1120_RESULT_STATUS_C;
	} else if (p_data->thrhigh_orig + M1120_DATA_REDUNDANCY <= raw) {
		status = M1120_RESULT_STATUS_A;
	} else {
		status = M1120_RESULT_STATUS_B;
	}
...
static int m1120_get_current_status(struct device *dev, struct m1120_raw_data *p_raw_data)
{
	...
	mdelay(20);
	err = m1120_measure_drdy(p_data, &raw);
	pr_info("m1120_get_current_status : rawData--== %6d\n", raw);
	p_raw_data->rawdata = raw;
	if (!err) {
		p_raw_data->status = m1120_get_result_status_for_PV(p_data, raw);
	} else {
		mxerr(&client->dev, "read raw data error, ret %d", err);
	}
...
static long m1120_misc_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	short kcalbuf = 0;
	int caldata = 0;
	struct m1120_raw_data m1120_data;
	...
	case M1120_IOCTL_SET_CALIBRATION:
		...
		*/
		...
		if (kbuf == 1) {/* First: 180 Degree Calibration */
			...
			} else if (p_m1120_data->calibrated_data > 0) {
				p_m1120_data->calibrated_data_polarity = 1;
			} else {
				p_m1120_data->calibrated_data_polarity = 0;
			}
			p_m1120_data->calibrated_HighFlag = 1;
			if (p_m1120_data->calibrated_data_polarity == 0) {
				p_m1120_data->thrlow = 0;
			} else {
			...
			pr_info("set calibrated data (%d)\n", p_m1120_data->calibrated_data);
			if (p_m1120_data->calibrated_data < 0) {
				p_m1120_data->thrlow = p_m1120_data->calibrated_data;
			} else {
				p_m1120_data->thrhigh = p_m1120_data->calibrated_data;
			}
		}
	...
	case M1120_IOCTL_GET_THRESHOLD_HIGH:
		kcalbuf = p_m1120_data->thrhigh_orig;
	...
	case M1120_IOCTL_SET_THRESHOLD_LOW:
		...
		pr_info("M1120_IOCTL_SET_THRESHOLD_LOW(%d)\n", kcalbuf);
	...
	case M1120_IOCTL_SET_CALIBRATION_POLARITY:
		...
		pr_info("M1120_IOCTL_SET_CALIBRATION_POLARITY(%d)\n", kbuf);
	...
	case M1120_IOCTL_GET_CALIBRATION_POLARITY:
		...
		break;
	case M1120_IOCTL_GET_CURRENT_STATUS:
		err = m1120_get_current_status(&p_m1120_data->client->dev, &m1120_data);
		if (err) {
			return err;
		}
		dbg("M1120_IOCTL_GET_CURRENT_STATUS(%d  %d)\n", m1120_data.status, m1120_data.rawdata);
		if (copy_to_user(argp, &m1120_data, sizeof(m1120_data))) {
			return -EFAULT;
		}
		break;
	case M1120_IOCTL_GET_HALLSWITCH_STATUS:
		kbuf = gpio_get_value(p_m1120_data->gpio_hallswitch);
		dbg("M1120_IOCTL_GET_HALLSWITCH_STATUS(%d)\n", kbuf);
		if (copy_to_user(argp, &kbuf, sizeof(kbuf))) {
			return -EFAULT;
		}
		break;
...
static ssize_t m1120_mode_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 64, "0x%x\n", m1120_get_mode(dev));
}

static ssize_t m1120_mode_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret = 0;
	...
	if ((mode == M1120_DETECTION_MODE_POLLING) || (mode == M1120_DETECTION_MODE_INTERRUPT)) {
		m1120_set_detection_mode(dev, mode);
	}
...
static DEVICE_ATTR(mode,	S_IRUGO|S_IWUSR|S_IWGRP, m1120_mode_show,	m1120_mode_store);
...
int m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	p_data->gpio_hallswitch = 1; /* GPIO: 1 */
	if (p_data->gpio_hallswitch != -1) {
		err = gpio_request(p_data->gpio_hallswitch, "m1120_irq_hallswitch");
		if (err) {
			mxerr(&client->dev, "gpio_hallswitch_request was failed(%d)", err);
			goto error_2;
		}
		mxinfo(&client->dev, "gpio_hallswitch_request was success");
		err = gpio_direction_input(p_data->gpio_hallswitch);
		if (err < 0) {
			mxerr(&client->dev, "gpio_hallswitch_direction_input was failed(%d)", err);
			goto error_2_1;
		}
		mxinfo(&client->dev, "gpio_hallswitch_direction_input was success");
		p_data->irq_hallswitch = gpio_to_irq(p_data->gpio_hallswitch);
	}
	...
	INIT_DELAYED_WORK(&p_data->work_hallswitch, m1120_work_hallswitch_func);
	...
	wake_lock_init(&p_data->wake_lock, WAKE_LOCK_SUSPEND, "m1120_wakelock");
...
error_2_1:
	if (p_data->gpio_hallswitch != -1) {
			gpio_free(p_data->gpio_hallswitch);
	}
error_2:
	if (p_data->igpio != -1) {
...
static int m1120_i2c_drv_remove(struct i2c_client *client)
{
	...
	wake_lock_destroy(&p_data->wake_lock);
