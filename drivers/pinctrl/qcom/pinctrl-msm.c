...
#define MSM_FP_SPI_MOSI_NUM	81
#define MSM_FP_SPI_MISO_NUM	82
#define MSM_FP_SPI_CS_NUM	83
#define MSM_FP_SPI_SCLK_NUM	84
...
static void msm_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	for (i = 0; i < chip->ngpio; i++, gpio++) {
		if ((i == MSM_FP_SPI_MOSI_NUM) || (i == MSM_FP_SPI_MISO_NUM)
				|| (i == MSM_FP_SPI_CS_NUM) || (i == MSM_FP_SPI_SCLK_NUM)) {
			pr_info("%s gpio %d controlled by TZ, so skip dump\n", __func__, i);
			continue;
		}
