...
#define FW_IMAGE_NAME_TD4322 "synaptics/startup_fw_update_td4322.img"
#define FW_IMAGE_NAME_S3330 "synaptics/startup_fw_update_s3330.img"
/*
...
static struct device_attribute attrs[] = {
	__ATTR(dorecovery_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_do_recovery_store),
	__ATTR(doreflash_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_image_name_store),
	__ATTR(imagesize_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_image_size_store),
	__ATTR(blocksize_2nd, S_IRUGO,
			fwu_sysfs_block_size_show,
			synaptics_rmi4_store_error),
	__ATTR(fwblockcount_2nd, S_IRUGO,
			fwu_sysfs_firmware_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(configblockcount_2nd, S_IRUGO,
			fwu_sysfs_configuration_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(dispconfigblockcount_2nd, S_IRUGO,
			fwu_sysfs_disp_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(permconfigblockcount_2nd, S_IRUGO,
			fwu_sysfs_perm_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(blconfigblockcount_2nd, S_IRUGO,
			fwu_sysfs_bl_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(uppblockcount_2nd, S_IRUGO,
			fwu_sysfs_utility_parameter_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(guestcodeblockcount_2nd, S_IRUGO,
			fwu_sysfs_guest_code_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(writeguestcode_2nd, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			fwu_sysfs_write_guest_code_store),
#ifdef SYNA_TDDI
	__ATTR(lockdowncode_2nd, (S_IWUSR | S_IWGRP | S_IRUGO),
			fwu_sysfs_read_lockdown_code_show,
			fwu_sysfs_write_lockdown_code_store),
#endif
};

static struct synaptics_rmi4_fwu_handle *fwu_2nd;
static char global_filename[64];

DECLARE_COMPLETION(fwu_remove_complete_2nd);

DEFINE_MUTEX(fwu_sysfs_mutex_2nd);
...
static int fwu_f51_force_data_init(void)
{
	...
	fwu_2nd->cal_data_off = offset[0] | offset[1] << 8;
	...
	if (query_count >= 10) {
		...
		if (packet_info & MASK_1BIT) {
			fwu_2nd->cal_packet_data_size = packet_info >> 1;
			fwu_2nd->cal_packet_data_size *= 2;
		} else {
			fwu_2nd->cal_packet_data_size = 0;
		}
	} else {
		fwu_2nd->cal_packet_data_size = 0;
	}

	fwu_2nd->cal_data_size = CAL_DATA_SIZE + fwu_2nd->cal_packet_data_size;
	if (fwu_2nd->cal_data_size > fwu_2nd->cal_data_buf_size) {
		kfree(fwu_2nd->cal_data);
		fwu_2nd->cal_data_buf_size = fwu_2nd->cal_data_size;
		fwu_2nd->cal_data = kmalloc(fwu_2nd->cal_data_buf_size, GFP_KERNEL);
		if (!fwu_2nd->cal_data) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to alloc mem for fwu_2nd->cal_data\n",
					__func__);
			fwu_2nd->cal_data_buf_size = 0;
			return -ENOMEM;
		}
	}
...
static int fwu_allocate_read_config_buf(unsigned int count)
{
	...
	if (count > fwu_2nd->read_config_buf_size) {
		kfree(fwu_2nd->read_config_buf);
		fwu_2nd->read_config_buf = kzalloc(count, GFP_KERNEL);
		if (!fwu_2nd->read_config_buf) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to alloc mem for fwu_2nd->read_config_buf\n",
					__func__);
			fwu_2nd->read_config_buf_size = 0;
			return -ENOMEM;
		}
		fwu_2nd->read_config_buf_size = count;
	}
...
{
	fwu_2nd->incompatible_partition_tables = false;

	if (fwu_2nd->phyaddr.bl_image != fwu_2nd->img.phyaddr.bl_image)
		fwu_2nd->incompatible_partition_tables = true;
	else if (fwu_2nd->phyaddr.lockdown != fwu_2nd->img.phyaddr.lockdown)
		fwu_2nd->incompatible_partition_tables = true;
	else if (fwu_2nd->phyaddr.bl_config != fwu_2nd->img.phyaddr.bl_config)
		fwu_2nd->incompatible_partition_tables = true;
	else if (fwu_2nd->phyaddr.utility_param != fwu_2nd->img.phyaddr.utility_param)
		fwu_2nd->incompatible_partition_tables = true;

	if (fwu_2nd->bl_version == BL_V7) {
		if (fwu_2nd->phyaddr.fl_config != fwu_2nd->img.phyaddr.fl_config)
			fwu_2nd->incompatible_partition_tables = true;
	}

	fwu_2nd->new_partition_table = false;

	if (fwu_2nd->phyaddr.ui_firmware != fwu_2nd->img.phyaddr.ui_firmware)
		fwu_2nd->new_partition_table = true;
	else if (fwu_2nd->phyaddr.ui_config != fwu_2nd->img.phyaddr.ui_config)
		fwu_2nd->new_partition_table = true;

	if (fwu_2nd->flash_properties.has_disp_config) {
		if (fwu_2nd->phyaddr.dp_config != fwu_2nd->img.phyaddr.dp_config)
			fwu_2nd->new_partition_table = true;
	}

	if (fwu_2nd->has_guest_code) {
		if (fwu_2nd->phyaddr.guest_code != fwu_2nd->img.phyaddr.guest_code)
			fwu_2nd->new_partition_table = true;
	}
		...
		struct block_count *blkcount, struct physical_address *phyaddr)
{
	...
	for (ii = 0; ii < fwu_2nd->partitions; ii++) {
		index = ii * 8 + 2;
...
static void fwu_parse_image_header_10_utility(const unsigned char *image)
{
	...
	num_of_containers = fwu_2nd->img.utility.size / 4;

	for (ii = 0; ii < num_of_containers; ii++) {
		if (ii >= MAX_UTILITY_PARAMS)
			continue;
		addr = le_to_uint(fwu_2nd->img.utility.data + (ii * 4));
		...
		case UTILITY_PARAMETER_CONTAINER:
			fwu_2nd->img.utility_param[ii].data = content;
			fwu_2nd->img.utility_param[ii].size = length;
			fwu_2nd->img.utility_param_id[ii] = content[0];
			break;
		default:
			break;
		};
...
static void fwu_parse_image_header_10_bootloader(const unsigned char *image)
{
	...
	num_of_containers = (fwu_2nd->img.bootloader.size - 4) / 4;

	for (ii = 1; ii <= num_of_containers; ii++) {
		addr = le_to_uint(fwu_2nd->img.bootloader.data + (ii * 4));
		descriptor = (struct container_descriptor *)(image + addr);
		container_id = descriptor->container_id[0] |
				descriptor->container_id[1] << 8;
		content = image + le_to_uint(descriptor->content_address);
		...
		case BL_IMAGE_CONTAINER:
			fwu_2nd->img.bl_image.data = content;
			fwu_2nd->img.bl_image.size = length;
			break;
		case BL_CONFIG_CONTAINER:
		case GLOBAL_PARAMETERS_CONTAINER:
			fwu_2nd->img.bl_config.data = content;
			fwu_2nd->img.bl_config.size = length;
			break;
		case BL_LOCKDOWN_INFO_CONTAINER:
		case DEVICE_CONFIG_CONTAINER:
			fwu_2nd->img.lockdown.data = content;
			fwu_2nd->img.lockdown.size = length;
			break;
		default:
			break;
		};
...
static void fwu_parse_image_header_10(void)
{
	...
	fwu_2nd->img.checksum = le_to_uint(header->checksum);
	...
	for (ii = 0; ii < num_of_containers; ii++) {
		...
		case CORE_CODE_CONTAINER:
			fwu_2nd->img.ui_firmware.data = content;
			fwu_2nd->img.ui_firmware.size = length;
			break;
		case UI_CONFIG_CONTAINER:
		case CORE_CONFIG_CONTAINER:
			fwu_2nd->img.ui_config.data = content;
			fwu_2nd->img.ui_config.size = length;
			break;
		case BL_CONTAINER:
			fwu_2nd->img.bl_version = *content;
			fwu_2nd->img.bootloader.data = content;
			fwu_2nd->img.bootloader.size = length;
			fwu_parse_image_header_10_bootloader(image);
			break;
		case UTILITY_CONTAINER:
			fwu_2nd->img.utility.data = content;
			fwu_2nd->img.utility.size = length;
			fwu_parse_image_header_10_utility(image);
			break;
		case GUEST_CODE_CONTAINER:
			fwu_2nd->img.contains_guest_code = true;
			fwu_2nd->img.guest_code.data = content;
			fwu_2nd->img.guest_code.size = length;
			break;
		case DISPLAY_CONFIG_CONTAINER:
			fwu_2nd->img.contains_disp_config = true;
			fwu_2nd->img.dp_config.data = content;
			fwu_2nd->img.dp_config.size = length;
			break;
		case PERMANENT_CONFIG_CONTAINER:
		case GUEST_SERIALIZATION_CONTAINER:
			fwu_2nd->img.contains_perm_config = true;
			fwu_2nd->img.pm_config.data = content;
			fwu_2nd->img.pm_config.size = length;
			break;
		case FLASH_CONFIG_CONTAINER:
			fwu_2nd->img.contains_flash_config = true;
			fwu_2nd->img.fl_config.data = content;
			fwu_2nd->img.fl_config.size = length;
			break;
		case GENERAL_INFORMATION_CONTAINER:
			fwu_2nd->img.contains_firmware_id = true;
			fwu_2nd->img.firmware_id = le_to_uint(content + 4);
			break;
		default:
			break;
		}
...
static void fwu_parse_image_header_05_06(void)
{
	...
	fwu_2nd->img.bl_version = header->header_version;

	fwu_2nd->img.contains_bootloader = header->options_bootloader;
	if (fwu_2nd->img.contains_bootloader)
		fwu_2nd->img.bootloader_size = le_to_uint(header->bootloader_size);

	fwu_2nd->img.ui_firmware.size = le_to_uint(header->firmware_size);
	if (fwu_2nd->img.ui_firmware.size) {
		fwu_2nd->img.ui_firmware.data = image + IMAGE_AREA_OFFSET;
		if (fwu_2nd->img.contains_bootloader)
			fwu_2nd->img.ui_firmware.data += fwu_2nd->img.bootloader_size;
	}

	if ((fwu_2nd->img.bl_version == BL_V6) && header->options_tddi)
		fwu_2nd->img.ui_firmware.data = image + IMAGE_AREA_OFFSET;

	fwu_2nd->img.ui_config.size = le_to_uint(header->config_size);
	if (fwu_2nd->img.ui_config.size) {
		fwu_2nd->img.ui_config.data = fwu_2nd->img.ui_firmware.data +
				fwu_2nd->img.ui_firmware.size;
	}

	if (fwu_2nd->img.contains_bootloader || header->options_tddi)
		fwu_2nd->img.contains_disp_config = true;
	else
		fwu_2nd->img.contains_disp_config = false;

	if (fwu_2nd->img.contains_disp_config) {
		fwu_2nd->img.disp_config_offset = le_to_uint(header->dsp_cfg_addr);
		fwu_2nd->img.dp_config.size = le_to_uint(header->dsp_cfg_size);
		fwu_2nd->img.dp_config.data = image + fwu_2nd->img.disp_config_offset;
	} else {
		retval = secure_memcpy(fwu_2nd->img.cstmr_product_id,
				sizeof(fwu_2nd->img.cstmr_product_id),
		...
		fwu_2nd->img.cstmr_product_id[PRODUCT_ID_SIZE] = 0;
	}

	fwu_2nd->img.contains_firmware_id = header->options_firmware_id;
	if (fwu_2nd->img.contains_firmware_id)
		fwu_2nd->img.firmware_id = le_to_uint(header->firmware_id);

	retval = secure_memcpy(fwu_2nd->img.product_id,
			sizeof(fwu_2nd->img.product_id),
			header->product_id,
	...
	fwu_2nd->img.product_id[PRODUCT_ID_SIZE] = 0;

	fwu_2nd->img.lockdown.size = LOCKDOWN_SIZE;
	fwu_2nd->img.lockdown.data = image + IMAGE_AREA_OFFSET - LOCKDOWN_SIZE;
...
static int fwu_parse_image_info(void)
{
	...
	header = (struct image_header_10 *)fwu_2nd->image;

	memset(&fwu_2nd->img, 0x00, sizeof(fwu_2nd->img));
	...
	if (fwu_2nd->bl_version == BL_V7 || fwu_2nd->bl_version == BL_V8) {
		if (!fwu_2nd->img.contains_flash_config) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		fwu_parse_partition_table(fwu_2nd->img.fl_config.data,
				&fwu_2nd->img.blkcount, &fwu_2nd->img.phyaddr);

		if (fwu_2nd->img.blkcount.utility_param)
			fwu_2nd->img.contains_utility_param = true;
...
static int fwu_read_flash_status(void)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fwu_2nd->f34_fd.data_base_addr + fwu_2nd->off.flash_status,
			&status,
			sizeof(status));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	fwu_2nd->in_bl_mode = status >> 7;

	if (fwu_2nd->bl_version == BL_V5)
		fwu_2nd->flash_status = (status >> 4) & MASK_3BIT;
	else if (fwu_2nd->bl_version == BL_V6)
		fwu_2nd->flash_status = status & MASK_3BIT;
	else if (fwu_2nd->bl_version == BL_V7 || fwu_2nd->bl_version == BL_V8)
		fwu_2nd->flash_status = status & MASK_5BIT;

	if (fwu_2nd->write_bootloader)
		fwu_2nd->flash_status = 0x00;

	if (fwu_2nd->flash_status != 0x00) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Flash status = %d, command = 0x%02x\n",
				__func__, fwu_2nd->flash_status, fwu_2nd->command);
	}

	if (fwu_2nd->bl_version == BL_V7 || fwu_2nd->bl_version == BL_V8) {
		if (fwu_2nd->flash_status == 0x08)
			fwu_2nd->flash_status = 0x00;
	}

	retval = synaptics_rmi4_reg_read(rmi4_data,
			fwu_2nd->f34_fd.data_base_addr + fwu_2nd->off.flash_cmd,
			&command,
			sizeof(command));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->bl_version == BL_V5)
		fwu_2nd->command = command & MASK_4BIT;
	else if (fwu_2nd->bl_version == BL_V6)
		fwu_2nd->command = command & MASK_6BIT;
	else if (fwu_2nd->bl_version == BL_V7 || fwu_2nd->bl_version == BL_V8)
		fwu_2nd->command = command;

	if (fwu_2nd->write_bootloader)
		fwu_2nd->command = 0x00;
...
static int fwu_wait_for_idle(int timeout_ms, bool poll)
{
	...
	do {
		...
		if ((fwu_2nd->command == CMD_IDLE) && (fwu_2nd->flash_status == 0x00))
			return 0;
...
static int fwu_write_f34_v7_command_single_transaction(unsigned char cmd)
{
	...
	data_base = fwu_2nd->f34_fd.data_base_addr;
	...
	data_1_5.payload_0 = fwu_2nd->bootloader_id[0];
	data_1_5.payload_1 = fwu_2nd->bootloader_id[1];

	retval = synaptics_rmi4_reg_write(rmi4_data,
			data_base + fwu_2nd->off.partition_id,
			data_1_5.data,
...
static int fwu_write_f34_v7_command(unsigned char cmd)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			data_base + fwu_2nd->off.flash_cmd,
			&command,
			sizeof(command));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_write_f34_v5v6_command(unsigned char cmd)
{
	...
	case CMD_ENABLE_FLASH_PROG:
		retval = synaptics_rmi4_reg_write(rmi4_data,
				data_base + fwu_2nd->off.payload,
				fwu_2nd->bootloader_id,
				sizeof(fwu_2nd->bootloader_id));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_write_f34_v7_partition_id(unsigned char cmd)
{
	...
	case CMD_WRITE_CONFIG:
	case CMD_READ_CONFIG:
		if (fwu_2nd->config_area == UI_CONFIG_AREA)
			partition = CORE_CONFIG_PARTITION;
		else if (fwu_2nd->config_area == DP_CONFIG_AREA)
			partition = DISPLAY_CONFIG_PARTITION;
		else if (fwu_2nd->config_area == PM_CONFIG_AREA)
			partition = GUEST_SERIALIZATION_PARTITION;
		else if (fwu_2nd->config_area == BL_CONFIG_AREA)
			partition = GLOBAL_PARAMETERS_PARTITION;
		else if (fwu_2nd->config_area == FLASH_CONFIG_AREA)
			partition = FLASH_CONFIG_PARTITION;
		else if (fwu_2nd->config_area == UPP_AREA)
...
static int fwu_read_f34_v7_partition_table(unsigned char *partition_table)
{
	...
	fwu_2nd->config_area = FLASH_CONFIG_AREA;
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			data_base + fwu_2nd->off.block_number,
			(unsigned char *)&block_number,
			sizeof(block_number));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	length[0] = (unsigned char)(fwu_2nd->flash_config_length & MASK_8BIT);
	length[1] = (unsigned char)(fwu_2nd->flash_config_length >> 8);

	retval = synaptics_rmi4_reg_write(rmi4_data,
			data_base + fwu_2nd->off.transfer_length,
			length,
			sizeof(length));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_read_f34_v7_queries(void)
{
	...
	query_base = fwu_2nd->f34_fd.query_base_addr;
	...
	fwu_2nd->bootloader_id[0] = query_1_7.bl_minor_revision;
	fwu_2nd->bootloader_id[1] = query_1_7.bl_major_revision;

	if (fwu_2nd->bootloader_id[1] == BL_V8)
		fwu_2nd->bl_version = BL_V8;

	fwu_2nd->block_size = query_1_7.block_size_15_8 << 8 |
			query_1_7.block_size_7_0;

	fwu_2nd->flash_config_length = query_1_7.flash_config_length_15_8 << 8 |
			query_1_7.flash_config_length_7_0;

	fwu_2nd->payload_length = query_1_7.payload_length_15_8 << 8 |
			query_1_7.payload_length_7_0;

	fwu_2nd->off.flash_status = V7_FLASH_STATUS_OFFSET;
	fwu_2nd->off.partition_id = V7_PARTITION_ID_OFFSET;
	fwu_2nd->off.block_number = V7_BLOCK_NUMBER_OFFSET;
	fwu_2nd->off.transfer_length = V7_TRANSFER_LENGTH_OFFSET;
	fwu_2nd->off.flash_cmd = V7_COMMAND_OFFSET;
	fwu_2nd->off.payload = V7_PAYLOAD_OFFSET;

	index = sizeof(query_1_7.data) - V7_PARTITION_SUPPORT_BYTES;

	fwu_2nd->partitions = 0;
	...
	fwu_2nd->partition_table_bytes = fwu_2nd->partitions * 8 + 2;

	ptable = kzalloc(fwu_2nd->partition_table_bytes, GFP_KERNEL);
	if (!ptable) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	fwu_parse_partition_table(ptable, &fwu_2nd->blkcount, &fwu_2nd->phyaddr);

	if (fwu_2nd->blkcount.dp_config)
		fwu_2nd->flash_properties.has_disp_config = 1;
	else
		fwu_2nd->flash_properties.has_disp_config = 0;

	if (fwu_2nd->blkcount.pm_config)
		fwu_2nd->flash_properties.has_pm_config = 1;
	else
		fwu_2nd->flash_properties.has_pm_config = 0;

	if (fwu_2nd->blkcount.bl_config)
		fwu_2nd->flash_properties.has_bl_config = 1;
	else
		fwu_2nd->flash_properties.has_bl_config = 0;

	if (fwu_2nd->blkcount.guest_code)
		fwu_2nd->has_guest_code = 1;
	else
		fwu_2nd->has_guest_code = 0;

	if (fwu_2nd->blkcount.utility_param)
		fwu_2nd->has_utility_param = 1;
	else
		fwu_2nd->has_utility_param = 0;
...
static int fwu_read_f34_v5v6_queries(void)
{
	...
	base = fwu_2nd->f34_fd.query_base_addr;
	...
	if (fwu_2nd->bl_version == BL_V5) {
		fwu_2nd->off.properties = V5_PROPERTIES_OFFSET;
		fwu_2nd->off.block_size = V5_BLOCK_SIZE_OFFSET;
		fwu_2nd->off.block_count = V5_BLOCK_COUNT_OFFSET;
		fwu_2nd->off.block_number = V5_BLOCK_NUMBER_OFFSET;
		fwu_2nd->off.payload = V5_BLOCK_DATA_OFFSET;
	} else if (fwu_2nd->bl_version == BL_V6) {
		fwu_2nd->off.properties = V6_PROPERTIES_OFFSET;
		fwu_2nd->off.properties_2 = V6_PROPERTIES_2_OFFSET;
		fwu_2nd->off.block_size = V6_BLOCK_SIZE_OFFSET;
		fwu_2nd->off.block_count = V6_BLOCK_COUNT_OFFSET;
		fwu_2nd->off.gc_block_count = V6_GUEST_CODE_BLOCK_COUNT_OFFSET;
		fwu_2nd->off.block_number = V6_BLOCK_NUMBER_OFFSET;
		fwu_2nd->off.payload = V6_BLOCK_DATA_OFFSET;
	}

	retval = synaptics_rmi4_reg_read(rmi4_data,
			base + fwu_2nd->off.block_size,
			buf,
			2);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	batohs(&fwu_2nd->block_size, &(buf[0]));

	if (fwu_2nd->bl_version == BL_V5) {
		fwu_2nd->off.flash_cmd = fwu_2nd->off.payload + fwu_2nd->block_size;
		fwu_2nd->off.flash_status = fwu_2nd->off.flash_cmd;
	} else if (fwu_2nd->bl_version == BL_V6) {
		fwu_2nd->off.flash_cmd = V6_FLASH_COMMAND_OFFSET;
		fwu_2nd->off.flash_status = V6_FLASH_STATUS_OFFSET;
	}

	retval = synaptics_rmi4_reg_read(rmi4_data,
			base + fwu_2nd->off.properties,
			fwu_2nd->flash_properties.data,
			sizeof(fwu_2nd->flash_properties.data));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->flash_properties.has_pm_config)
		count += 2;

	if (fwu_2nd->flash_properties.has_bl_config)
		count += 2;
	...
	batohs(&fwu_2nd->blkcount.ui_firmware, &(buf[0]));
	batohs(&fwu_2nd->blkcount.ui_config, &(buf[2]));

	count = 4;

	if (fwu_2nd->flash_properties.has_pm_config) {
		batohs(&fwu_2nd->blkcount.pm_config, &(buf[count]));
		count += 2;
	}

	if (fwu_2nd->flash_properties.has_bl_config) {
		batohs(&fwu_2nd->blkcount.bl_config, &(buf[count]));
		count += 2;
	}

	if (fwu_2nd->flash_properties.has_disp_config)
		batohs(&fwu_2nd->blkcount.dp_config, &(buf[count]));

	fwu_2nd->has_guest_code = false;
#ifdef SYNA_TDDI
	fwu_2nd->has_force_config = false;
	fwu_2nd->has_lockdown_data = false;
	fwu_2nd->has_lcm_data = false;
	fwu_2nd->has_oem_data = false;
#endif

	if (fwu_2nd->flash_properties.has_query4) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
				base + fwu_2nd->off.properties_2,
				properties_2.data,
		...
		offset = fwu_2nd->off.properties_2 + 1;
		count = 0;
		if (properties_2.has_guest_code) {
			...
			batohs(&fwu_2nd->blkcount.guest_code, &(buf[0]));
			count++;
			fwu_2nd->has_guest_code = true;
		}
#ifdef SYNA_TDDI
		if (properties_2.has_force_config) {
			...
			batohs(&fwu_2nd->blkcount.tddi_force_config, &(buf[0]));
			count++;
			fwu_2nd->has_force_config = true;
		}
		if (properties_2.has_lockdown_data) {
			...
			batohs(&fwu_2nd->blkcount.tddi_lockdown_data, &(buf[0]));
			count++;
			fwu_2nd->has_lockdown_data = true;
		}
		if (properties_2.has_lcm_data) {
			...
			batohs(&fwu_2nd->blkcount.tddi_lcm_data, &(buf[0]));
			count++;
			fwu_2nd->has_lcm_data = true;
		}
		if (properties_2.has_oem_data) {
			...
			batohs(&fwu_2nd->blkcount.tddi_oem_data, &(buf[0]));
			fwu_2nd->has_oem_data = true;
		}
#endif
	}

	fwu_2nd->has_utility_param = false;
...
static int fwu_read_f34_queries(void)
{
	...
	memset(&fwu_2nd->blkcount, 0x00, sizeof(fwu_2nd->blkcount));
	memset(&fwu_2nd->phyaddr, 0x00, sizeof(fwu_2nd->phyaddr));
		...
		unsigned short block_cnt, unsigned char command)
{
	...
	do {
		if (remaining / fwu_2nd->payload_length)
			transfer = fwu_2nd->payload_length;
		else
			transfer = remaining;
		...
		if (max_write_size >= transfer * fwu_2nd->block_size)
			max_write_size = transfer * fwu_2nd->block_size;
		else if (max_write_size > fwu_2nd->block_size)
			max_write_size -= max_write_size % fwu_2nd->block_size;
		else
			max_write_size = fwu_2nd->block_size;
#else
		max_write_size = transfer * fwu_2nd->block_size;
#endif
		left_bytes = transfer * fwu_2nd->block_size;
		...
		unsigned short block_cnt, unsigned char command)
{
	...
	block_number[1] |= (fwu_2nd->config_area << 5);
		...
		unsigned char command)
{
	...
	do {
		...
		retval = synaptics_rmi4_reg_read(rmi4_data,
				data_base + fwu_2nd->off.payload,
				&fwu_2nd->read_config_buf[index],
				transfer * fwu_2nd->block_size);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		index += (transfer * fwu_2nd->block_size);
		remaining -= transfer;
	} while (remaining);
...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	if (fwu_2nd->img.contains_firmware_id) {
		*fw_id = fwu_2nd->img.firmware_id;
	} else {
		strptr = strnstr(fwu_2nd->image_name, "PR", MAX_IMAGE_NAME_LEN);
		if (!strptr) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: No valid PR number (PRxxxxxxx) found in image file name (%s)\n",
					__func__, fwu_2nd->image_name);
			return -EINVAL;
		}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (fwu_2nd->force_update) {
		flash_area = UI_FIRMWARE;
		goto exit;
	}

	/* Update both UI and config if device is in bootloader mode */
	if (fwu_2nd->bl_mode_device) {
		flash_area = UI_FIRMWARE;
		goto exit;
	}
	...
	SYNA_INFO("%s: Device firmware ID = %d\n",
	...
	SYNA_INFO("%s: Image firmware ID = %d\n",
	...
	for (ii = 0; ii < config_id_size; ii++) {
		SYNA_INFO("%d: img = 0x%02X, config = 0x%02X\n",
			ii, fwu_2nd->img.ui_config.data[ii], fwu_2nd->config_id[ii]);
		if (fwu_2nd->img.ui_config.data[ii] > fwu_2nd->config_id[ii]) {
			flash_area = UI_CONFIG;
			goto exit;
		} else if (fwu_2nd->img.ui_config.data[ii] < fwu_2nd->config_id[ii]) {
			flash_area = NONE;
			goto exit;
		}
	}
...
exit:
	if (flash_area == NONE) {
		SYNA_INFO("%s: No need to do reflash\n",
				__func__);
	} else {
		SYNA_INFO("%s: Updating %s\n",
				__func__,
				flash_area == UI_FIRMWARE ?
...
static int fwu_scan_pdt(void)
{
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		...
		if (rmi_fd.fn_number) {
			...
			case SYNAPTICS_RMI4_F34:
				f34found = true;
				fwu_2nd->f34_fd.query_base_addr =
						rmi_fd.query_base_addr;
				fwu_2nd->f34_fd.ctrl_base_addr =
						rmi_fd.ctrl_base_addr;
				fwu_2nd->f34_fd.data_base_addr =
						rmi_fd.data_base_addr;

				switch (rmi_fd.fn_version) {
				case F34_V0:
					fwu_2nd->bl_version = BL_V5;
					break;
				case F34_V1:
					fwu_2nd->bl_version = BL_V6;
					break;
				case F34_V2:
					fwu_2nd->bl_version = BL_V7;
					break;
				default:
					dev_err(rmi4_data->pdev->dev.parent,
				...
				for (ii = intr_off;
						...
						ii++) {
					fwu_2nd->intr_mask |= 1 << ii;
				}
				break;
			case SYNAPTICS_RMI4_F35:
				f35found = true;
				fwu_2nd->f35_fd.query_base_addr =
						rmi_fd.query_base_addr;
				fwu_2nd->f35_fd.ctrl_base_addr =
						rmi_fd.ctrl_base_addr;
				fwu_2nd->f35_fd.data_base_addr =
						rmi_fd.data_base_addr;
				fwu_2nd->f35_fd.cmd_base_addr =
						rmi_fd.cmd_base_addr;
				break;
			}
		} else {
	...
	rmi4_data->intr_mask[0] |= fwu_2nd->intr_mask;
...
static int fwu_check_ui_firmware_size(void)
{
	...
	block_count = fwu_2nd->img.ui_firmware.size / fwu_2nd->block_size;

	if (block_count != fwu_2nd->blkcount.ui_firmware) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_check_ui_configuration_size(void)
{
	...
	block_count = fwu_2nd->img.ui_config.size / fwu_2nd->block_size;

	if (block_count != fwu_2nd->blkcount.ui_config) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_check_dp_configuration_size(void)
{
	...
	block_count = fwu_2nd->img.dp_config.size / fwu_2nd->block_size;

	if (block_count != fwu_2nd->blkcount.dp_config) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_check_pm_configuration_size(void)
{
	...
	block_count = fwu_2nd->img.pm_config.size / fwu_2nd->block_size;

	if (block_count != fwu_2nd->blkcount.pm_config) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_check_bl_configuration_size(void)
{
	...
	block_count = fwu_2nd->img.bl_config.size / fwu_2nd->block_size;

	if (block_count != fwu_2nd->blkcount.bl_config) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_check_guest_code_size(void)
{
	...
	block_count = fwu_2nd->img.guest_code.size / fwu_2nd->block_size;
	if (block_count != fwu_2nd->blkcount.guest_code) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_erase_all(void)
{
	...
	if (fwu_2nd->bl_version == BL_V7) {
		...
		fwu_2nd->config_area = UI_CONFIG_AREA;
		...
		retval = fwu_wait_for_idle(ERASE_WAIT_MS, false);
		if (!(fwu_2nd->bl_version == BL_V8 &&
				fwu_2nd->flash_status == BAD_PARTITION_TABLE)) {
			if (retval < 0)
				return retval;
		}
	...
	if (fwu_2nd->flash_properties.has_disp_config) {
		fwu_2nd->config_area = DP_CONFIG_AREA;
		retval = fwu_erase_configuration();
		if (retval < 0)
			return retval;
	}
...
static int fwu_write_firmware(void)
{
	...
	firmware_block_count = fwu_2nd->img.ui_firmware.size / fwu_2nd->block_size;

	return fwu_write_f34_blocks((unsigned char *)fwu_2nd->img.ui_firmware.data,
...
static int fwu_write_bootloader(void)
{
	...
	bootloader_block_count = fwu_2nd->img.bl_image.size / fwu_2nd->block_size;

	fwu_2nd->write_bootloader = true;
	retval = fwu_write_f34_blocks((unsigned char *)fwu_2nd->img.bl_image.data,
			bootloader_block_count, CMD_WRITE_BOOTLOADER);
	fwu_2nd->write_bootloader = false;
...
static int fwu_write_utility_parameter(void)
{
	...
	utility_param_size = fwu_2nd->blkcount.utility_param * fwu_2nd->block_size;
	retval = fwu_allocate_read_config_buf(utility_param_size);
	if (retval < 0)
		return retval;
	memset(fwu_2nd->read_config_buf, 0x00, utility_param_size);

	pbuf = fwu_2nd->read_config_buf;
	remaining_size = utility_param_size - 4;

	for (ii = 0; ii < MAX_UTILITY_PARAMS; ii++) {
		if (fwu_2nd->img.utility_param_id[ii] == UNUSED)
			continue;

#ifdef F51_DISCRETE_FORCE
		if (fwu_2nd->img.utility_param_id[ii] == FORCE_PARAMETER) {
			if (fwu_2nd->bl_mode_device) {
			...
			retval = secure_memcpy(&(pbuf[4]),
					remaining_size - 4,
					fwu_2nd->cal_data,
					fwu_2nd->cal_data_buf_size,
					fwu_2nd->cal_data_size);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
			...
			pbuf[1] = 0x00;
			pbuf[2] = (4 + fwu_2nd->cal_data_size) / 2;
			pbuf += (fwu_2nd->cal_data_size + 4);
			remaining_size -= (fwu_2nd->cal_data_size + 4);
			continue;
		}
image_param:
#endif
		...
		retval = secure_memcpy(pbuf,
				remaining_size,
				fwu_2nd->img.utility_param[ii].data,
				fwu_2nd->img.utility_param[ii].size,
				fwu_2nd->img.utility_param[ii].size);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		pbuf += fwu_2nd->img.utility_param[ii].size;
		remaining_size -= fwu_2nd->img.utility_param[ii].size;
	}

	calculate_checksum((unsigned short *)fwu_2nd->read_config_buf,
			((utility_param_size - 4) / 2),
			&checksum);

	convert_to_little_endian(checksum_array, checksum);

	fwu_2nd->read_config_buf[utility_param_size - 4] = checksum_array[0];
	fwu_2nd->read_config_buf[utility_param_size - 3] = checksum_array[1];
	fwu_2nd->read_config_buf[utility_param_size - 2] = checksum_array[2];
	fwu_2nd->read_config_buf[utility_param_size - 1] = checksum_array[3];

	retval = fwu_write_f34_blocks((unsigned char *)fwu_2nd->read_config_buf,
			fwu_2nd->blkcount.utility_param, CMD_WRITE_UTILITY_PARAM);
	if (retval < 0)
		return retval;
...
{
	return fwu_write_f34_blocks((unsigned char *)fwu_2nd->config_data,
			fwu_2nd->config_block_count, CMD_WRITE_CONFIG);
}

static int fwu_write_ui_configuration(void)
{
	fwu_2nd->config_area = UI_CONFIG_AREA;
	fwu_2nd->config_data = fwu_2nd->img.ui_config.data;
	fwu_2nd->config_size = fwu_2nd->img.ui_config.size;
	fwu_2nd->config_block_count = fwu_2nd->config_size / fwu_2nd->block_size;
...
{
	fwu_2nd->config_area = DP_CONFIG_AREA;
	fwu_2nd->config_data = fwu_2nd->img.dp_config.data;
	fwu_2nd->config_size = fwu_2nd->img.dp_config.size;
	fwu_2nd->config_block_count = fwu_2nd->config_size / fwu_2nd->block_size;
...
{
	fwu_2nd->config_area = PM_CONFIG_AREA;
	fwu_2nd->config_data = fwu_2nd->img.pm_config.data;
	fwu_2nd->config_size = fwu_2nd->img.pm_config.size;
	fwu_2nd->config_block_count = fwu_2nd->config_size / fwu_2nd->block_size;
...
static int fwu_write_tddi_lockdown_data(void)
{
	...
	retval = fwu_write_f34_blocks(fwu_2nd->read_config_buf,
			fwu_2nd->blkcount.tddi_lockdown_data,
			CMD_WRITE_LOCKDOWN_DATA);
	if (retval < 0)
		return retval;
...
static int fwu_write_flash_configuration(void)
{
	...
	fwu_2nd->config_area = FLASH_CONFIG_AREA;
	fwu_2nd->config_data = fwu_2nd->img.fl_config.data;
	fwu_2nd->config_size = fwu_2nd->img.fl_config.size;
	fwu_2nd->config_block_count = fwu_2nd->config_size / fwu_2nd->block_size;

	if (fwu_2nd->config_block_count != fwu_2nd->blkcount.fl_config) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_write_guest_code(void)
{
	...
	guest_code_block_count = fwu_2nd->img.guest_code.size / fwu_2nd->block_size;

	retval = fwu_write_f34_blocks((unsigned char *)fwu_2nd->img.guest_code.data,
...
static int fwu_write_lockdown(void)
{
	...
	lockdown_block_count = fwu_2nd->img.lockdown.size / fwu_2nd->block_size;

	return fwu_write_f34_blocks((unsigned char *)fwu_2nd->img.lockdown.data,
...
static int fwu_write_partition_table_v7(void)
{
	...
	block_count = fwu_2nd->blkcount.bl_config;
	fwu_2nd->config_area = BL_CONFIG_AREA;
	fwu_2nd->config_size = fwu_2nd->block_size * block_count;

	retval = fwu_allocate_read_config_buf(fwu_2nd->config_size);
	if (retval < 0)
		return retval;
	...
	fwu_2nd->config_area = BL_CONFIG_AREA;
	fwu_2nd->config_data = fwu_2nd->read_config_buf;
	fwu_2nd->config_size = fwu_2nd->img.bl_config.size;
	fwu_2nd->config_block_count = fwu_2nd->config_size / fwu_2nd->block_size;
...
static int fwu_write_bl_area_v7(void)
{
	...
	has_utility_param = fwu_2nd->has_utility_param;

	if (fwu_2nd->has_utility_param) {
		fwu_2nd->config_area = UPP_AREA;
		retval = fwu_erase_configuration();
		if (retval < 0)
			return retval;
	}
	...
	fwu_2nd->config_area = BL_CONFIG_AREA;
	fwu_2nd->config_data = fwu_2nd->img.bl_config.data;
	fwu_2nd->config_size = fwu_2nd->img.bl_config.size;
	fwu_2nd->config_block_count = fwu_2nd->config_size / fwu_2nd->block_size;
	retval = fwu_write_configuration();
	if (retval < 0)
	...
	if (fwu_2nd->img.contains_utility_param) {
...
static int fwu_do_reflash(void)
{
	...
	if (!fwu_2nd->new_partition_table) {
		...
		if (fwu_2nd->flash_properties.has_disp_config &&
				fwu_2nd->img.contains_disp_config) {
			retval = fwu_check_dp_configuration_size();
			if (retval < 0)
				return retval;
		}

		if (fwu_2nd->has_guest_code && fwu_2nd->img.contains_guest_code) {
			retval = fwu_check_guest_code_size();
			if (retval < 0)
				return retval;
		}
	} else if (fwu_2nd->bl_version == BL_V7) {
		retval = fwu_check_bl_configuration_size();
		if (retval < 0)
			return retval;
	}

	if (!fwu_2nd->has_utility_param && fwu_2nd->img.contains_utility_param) {
		if (fwu_2nd->bl_version == BL_V7 || fwu_2nd->bl_version == BL_V8)
			do_bl_update = true;
	}
	...
	if (!do_bl_update && fwu_2nd->incompatible_partition_tables) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Incompatible partition tables\n",
				__func__);
		return -EINVAL;
	} else if (!do_bl_update && fwu_2nd->new_partition_table) {
		if (!fwu_2nd->force_update) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	} else if (fwu_2nd->bl_version == BL_V7 && fwu_2nd->new_partition_table) {
		retval = fwu_write_partition_table_v7();
		if (retval < 0)
			return retval;
		pr_notice("%s: Partition table programmed\n", __func__);
	} else if (fwu_2nd->bl_version == BL_V8) {
...
static int fwu_do_read_config(void)
{
	...
	switch (fwu_2nd->config_area) {
	case UI_CONFIG_AREA:
		block_count = fwu_2nd->blkcount.ui_config;
		break;
	case DP_CONFIG_AREA:
		if (!fwu_2nd->flash_properties.has_disp_config) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		block_count = fwu_2nd->blkcount.dp_config;
		break;
	case PM_CONFIG_AREA:
		if (!fwu_2nd->flash_properties.has_pm_config) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		block_count = fwu_2nd->blkcount.pm_config;
		break;
	case BL_CONFIG_AREA:
		if (!fwu_2nd->flash_properties.has_bl_config) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	case UPP_AREA:
		...
		block_count = fwu_2nd->blkcount.utility_param;
		break;
#ifdef SYNA_TDDI
	case TDDI_FORCE_CONFIG_AREA:
		if (!fwu_2nd->has_force_config) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: force configuration not supported\n",
					__func__);
			return -EINVAL;
		}
		block_count = fwu_2nd->blkcount.tddi_force_config;
		break;
	case TDDI_OEM_DATA_AREA:
		if (!fwu_2nd->has_oem_data) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: oem data not supported\n",
					__func__);
			return -EINVAL;
		}
		block_count = fwu_2nd->blkcount.tddi_oem_data;
		break;
	case TDDI_LCM_DATA_AREA:
		if (!fwu_2nd->has_lcm_data) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: lcm data not supported\n",
					__func__);
			return -EINVAL;
		}
		block_count = fwu_2nd->blkcount.tddi_lcm_data;
		break;
#endif
	default:
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->bl_version == BL_V5 || fwu_2nd->bl_version == BL_V6) {
		config_area = fwu_2nd->config_area;
		retval = fwu_enter_flash_prog();
		fwu_2nd->config_area = config_area;
		if (retval < 0)
			goto exit;
	}
...
static int fwu_do_read_tddi_lockdown_data(void)
{
	...
	block_count = fwu_2nd->blkcount.tddi_lockdown_data;
	fwu_2nd->config_size = fwu_2nd->block_size * block_count;

	if (fwu_2nd->bl_version != BL_V6) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Not support lockdown data in bl v.%d\n",
				__func__,
				fwu_2nd->bl_version);
		goto exit;
	} else if (!fwu_2nd->has_lockdown_data) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	fwu_2nd->read_config_buf = kzalloc(fwu_2nd->config_size, GFP_KERNEL);
	...
	fwu_2nd->read_config_buf_size = fwu_2nd->config_size;
	retval = fwu_read_f34_blocks(block_count, CMD_READ_LOCKDOWN_DATA);
exit:
	return retval;
}

int get_tddi_lockdown_data_2nd(unsigned char *lockdown_data, unsigned short leng)
{
	int retval;
	...
	if (retval < 0)
		return retval;
	memcpy(lockdown_data, fwu_2nd->read_config_buf, leng);
	return retval;
}

int set_tddi_lockdown_data_2nd(unsigned char *lockdown_data, unsigned short leng)
{
	int retval = -EINVAL;
	unsigned long checksum;
	unsigned char checksum_array[4];
	...
	fwu_2nd->config_size = fwu_2nd->blkcount.tddi_lockdown_data * fwu_2nd->block_size;
	retval = fwu_allocate_read_config_buf(fwu_2nd->config_size);
	if (retval < 0)
		goto exit;
	memset(fwu_2nd->read_config_buf, 0x00, fwu_2nd->config_size);
	retval = secure_memcpy(fwu_2nd->read_config_buf, fwu_2nd->config_size,
	...
	calculate_checksum((unsigned short *)fwu_2nd->read_config_buf,
			((fwu_2nd->config_size - 4) / 2),
			&checksum);

	convert_to_little_endian(checksum_array, checksum);

	fwu_2nd->read_config_buf[blk_cnt * fwu_2nd->block_size - 4] = checksum_array[0];
	fwu_2nd->read_config_buf[blk_cnt * fwu_2nd->block_size - 3] = checksum_array[1];
	fwu_2nd->read_config_buf[blk_cnt * fwu_2nd->block_size - 2] = checksum_array[2];
	fwu_2nd->read_config_buf[blk_cnt * fwu_2nd->block_size - 1] = checksum_array[3];
...
static int fwu_do_lockdown_v5v6(void)
{
	...
	img_ld = (unsigned char *)fwu_2nd->img.lockdown.data;
	if (fwu_2nd->has_lockdown_data) {
		retval = set_tddi_lockdown_data_2nd(img_ld,
				LOCKDOWN_SIZE);
		if (retval < 0)
			dev_err(rmi4_data->pdev->dev.parent,
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fwu_2nd->f34_fd.query_base_addr + fwu_2nd->off.properties,
			fwu_2nd->flash_properties.data,
			sizeof(fwu_2nd->flash_properties.data));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->flash_properties.unlocked == 0) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Device already locked down\n",
				__func__);
		return 0;
...
static int fwu_do_restore_f51_cal_data(void)
{
	...
	retval = secure_memcpy(&fwu_2nd->read_config_buf[fwu_2nd->cal_data_off],
			fwu_2nd->cal_data_size, fwu_2nd->cal_data,
			fwu_2nd->cal_data_buf_size, fwu_2nd->cal_data_size);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	fwu_2nd->read_config_buf[fwu_2nd->config_size - 4] = checksum_array[0];
	fwu_2nd->read_config_buf[fwu_2nd->config_size - 3] = checksum_array[1];
	fwu_2nd->read_config_buf[fwu_2nd->config_size - 2] = checksum_array[2];
	fwu_2nd->read_config_buf[fwu_2nd->config_size - 1] = checksum_array[3];
...
static int fwu_start_write_guest_code(void)
{
	...
	if (!fwu_2nd->img.contains_guest_code) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_start_write_config(void)
{
	...
	case PM_CONFIG_AREA:
		...
		if (!fwu_2nd->img.contains_perm_config) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->config_area != PM_CONFIG_AREA) {
		retval = fwu_erase_configuration();
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_start_reflash(void)
{
	...
	if (fwu_2nd->image == NULL) {
		if (zte_ts_is_td4322())
			fw_name = FW_IMAGE_NAME_TD4322;
		else
			fw_name = FW_IMAGE_NAME_S3330;

		if (fwu_2nd->force_update == true)
			fw_name = global_filename;

		memset(fwu_2nd->image_name, 0, MAX_IMAGE_NAME_LEN);
		retval = secure_memcpy(fwu_2nd->image_name, MAX_IMAGE_NAME_LEN,
				fw_name, strlen(fw_name), strlen(fw_name));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		SYNA_INFO("%s: Requesting firmware image %s\n",
				__func__, fwu_2nd->image_name);

		retval = request_firmware(&fw_entry, fwu_2nd->image_name,
				rmi4_data->pdev->dev.parent);
		if (retval != 0) {
			SYNA_ERROR("%s: Firmware image %s not available\n",
					__func__, fwu_2nd->image_name);
			retval = -EINVAL;
			goto exit;
		}
	...
	if (fwu_2nd->blkcount.total_count != fwu_2nd->img.blkcount.total_count) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->bl_version != fwu_2nd->img.bl_version) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu_2nd->in_bl_mode) {
		fwu_2nd->bl_mode_device = true;
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Device in bootloader mode\n",
				__func__);
	} else {
		fwu_2nd->bl_mode_device = false;
	}
...
#ifdef F51_DISCRETE_FORCE
	if (flash_area != NONE && !fwu_2nd->bl_mode_device) {
		fwu_2nd->config_size = fwu_2nd->block_size * fwu_2nd->blkcount.ui_config;
		fwu_2nd->config_area = UI_CONFIG_AREA;
		...
		retval = fwu_read_f34_blocks(fwu_2nd->blkcount.ui_config,
				CMD_READ_CONFIG);
		if (retval < 0) {
			rmi4_data->reset_device(rmi4_data, false);
			goto exit;
		...
		retval = secure_memcpy(fwu_2nd->cal_data, fwu_2nd->cal_data_buf_size,
				&fwu_2nd->read_config_buf[fwu_2nd->cal_data_off],
				fwu_2nd->cal_data_size, fwu_2nd->cal_data_size);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	case UI_FIRMWARE:
		...
		if (fwu_2nd->bl_mode_device || fwu_2nd->in_bl_mode) {
			dev_info(rmi4_data->pdev->dev.parent,
					"%s: Device in bootloader mode, skipping calibration data restoration\n",
					__func__);
			break;
	...
	if (fwu_2nd->do_lockdown && (fwu_2nd->img.lockdown.data != NULL)) {
		switch (fwu_2nd->bl_version) {
		case BL_V5:
		case BL_V6:
...
static int fwu_recovery_check_status(void)
{
	...
	data_base = fwu_2nd->f35_fd.data_base_addr;
...
static int fwu_recovery_erase_all(void)
{
	...
	ctrl_base = fwu_2nd->f35_fd.ctrl_base_addr;
	...
	if (fwu_2nd->f35_fd.cmd_base_addr) {
...
static int fwu_recovery_write_chunk(void)
{
	...
	unsigned char *chunk_ptr = (unsigned char *)fwu_2nd->image;
	struct synaptics_rmi4_data *rmi4_data = fwu_2nd->rmi4_data;
	...
	chunk_total = fwu_2nd->image_size / F35_CHUNK_SIZE;
	chunk_spare = fwu_2nd->image_size % F35_CHUNK_SIZE;
	if (chunk_spare)
		chunk_total++;
	...
	for (chunk = 0; chunk < chunk_total; chunk++) {
		...
		secure_memcpy(buf, sizeof(buf), chunk_ptr,
					fwu_2nd->image_size - bytes_written,
					chunk_size);
...
static int fwu_start_recovery(void)
{
	...
	if (fwu_2nd->image == NULL) {
		...
		fwu_2nd->image = fw_entry->data;
		fwu_2nd->image_size = fw_entry->size;
	}
...
int synaptics_fw_updater_2nd(const unsigned char *fw_data)
{
	...
	if (!fwu_2nd->initialized)
		return -ENODEV;

	if (fwu_2nd->in_ub_mode) {
		fwu_2nd->image = NULL;
...
EXPORT_SYMBOL(synaptics_fw_updater_2nd);

#ifdef CONFIG_BOARD_FUJISAN
int zte_get_ts_power_status(const unsigned char *fw_data)
{
	struct synaptics_rmi4_data *rmi4_data = fwu_2nd->rmi4_data;

	if (rmi4_data->stay_awake == true)
		return 1;

	return 0;
}
EXPORT_SYMBOL(zte_get_ts_power_status);
#endif
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	if (!socinfo_get_normal_flag()) {
		SYNA_INFO("update firmware only in normal mode\n");
		goto FailedLoop;
	}
...
FailedLoop:
#ifdef CONFIG_BOARD_FUJISAN
	if (!rmi4_data->rebuild_flag) {
		SYNA_INFO("The rebuild process is not execute\n");
		synaptics_rmi4_suspend_public(0);
	}
#endif
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (!mutex_trylock(&fwu_sysfs_mutex_2nd))
		return -EBUSY;

	if (count < fwu_2nd->config_size) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	retval = secure_memcpy(buf, count, fwu_2nd->read_config_buf,
			fwu_2nd->read_config_buf_size, fwu_2nd->config_size);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
		...
		retval = fwu_2nd->config_size;
	}

exit:
	mutex_unlock(&fwu_sysfs_mutex_2nd);
	return retval;
}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	retval = secure_memcpy(&fwu_2nd->ext_data_source[fwu_2nd->data_pos],
			fwu_2nd->image_size - fwu_2nd->data_pos, buf, count, count);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fwu_2nd->ext_data_source) {
		retval = -EINVAL;
		goto exit;
	} else {
		fwu_2nd->image = fwu_2nd->ext_data_source;
	}
...
exit:
	kfree(fwu_2nd->ext_data_source);
	fwu_2nd->ext_data_source = NULL;
	fwu_2nd->image = NULL;
	mutex_unlock(&fwu_sysfs_mutex_2nd);
	return retval;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input == FORCE)
		fwu_2nd->force_update = true;

	retval = synaptics_fw_updater_2nd(fwu_2nd->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
exit:
	...
	fwu_2nd->ext_data_source = NULL;
	fwu_2nd->image = NULL;
	fwu_2nd->force_update = FORCE_UPDATE;
	fwu_2nd->do_lockdown = DO_LOCKDOWN;
	mutex_unlock(&fwu_sysfs_mutex_2nd);
	return retval;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fwu_2nd->image_size = size;
	fwu_2nd->data_pos = 0;

	kfree(fwu_2nd->ext_data_source);
	fwu_2nd->ext_data_source = kzalloc(fwu_2nd->image_size, GFP_KERNEL);
	if (!fwu_2nd->ext_data_source) {
		dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = snprintf(buf, PAGE_SIZE, "%u\n", fwu_2nd->blkcount.utility_param);
		...
		struct device_attribute *attr, char *buf)
{
	...
	lockdown_data_size = fwu_2nd->blkcount.tddi_lockdown_data * fwu_2nd->block_size;
	...
	if (get_tddi_lockdown_data_2nd(lockdown_data, lockdown_data_size) < 0) {
		kfree(lockdown_data);
		mutex_unlock(&fwu_sysfs_mutex_2nd);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (set_tddi_lockdown_data_2nd(lockdown_data, lockdown_data_size) < 0) {
		kfree(lockdown_data);
		mutex_unlock(&fwu_sysfs_mutex_2nd);
		return -EINVAL;
	}
...
#endif
static void test_check_file_name(char *buffer, unsigned int len)
{
	unsigned int i;
...
static ssize_t synaptics_ts_update_func(struct file *file, const char  __user *buffer,
...

{
	...
	if (sizeof(global_filename) < count) {
		SYNA_INFO("fliename is too long\n");
		retval = -EINVAL;
		goto exit;
	}

	memset(global_filename, 0, sizeof(global_filename));
	memcpy(global_filename, buffer, count);
	test_check_file_name(global_filename, sizeof(global_filename));
...
static const struct file_operations synaptics_ts_update = {
	.owner = THIS_MODULE,
	.write = synaptics_ts_update_func,
};
		...
		unsigned char intr_mask)
{
	...
	if (fwu_2nd->intr_mask & intr_mask)
		fwu_read_flash_status();
}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	fwu_2nd = kzalloc(sizeof(*fwu_2nd), GFP_KERNEL);
	if (!fwu_2nd) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to alloc mem for fwu_2nd\n",
				__func__);
		retval = -ENOMEM;
		goto exit;
	}

	fwu_2nd->image_name = kzalloc(MAX_IMAGE_NAME_LEN, GFP_KERNEL);
	if (!fwu_2nd->image_name) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	fwu_2nd->force_update = FORCE_UPDATE;
	fwu_2nd->do_lockdown = DO_LOCKDOWN;
	fwu_2nd->initialized = true;

#ifdef DO_STARTUP_FW_UPDATE
	fwu_2nd->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue2");
	INIT_WORK(&fwu_2nd->fwu_work, fwu_startup_fw_update_work);
	queue_work(fwu_2nd->fwu_workqueue,
			&fwu_2nd->fwu_work);

#endif
	res = proc_create("touchscreen/ts_update_2nd", 0664, NULL, &synaptics_ts_update);
	if (res) {
		SYNA_INFO("%s create node ts_update_2nd.\n", __func__);
	} else {
		SYNA_INFO("%s create node ts_update_2nd failed!!!\n", __func__);
	}
	...
	if (false == ENABLE_SYS_REFLASH) {
#ifdef ZTE_FASTMMI_MANUFACTURING_VERSION
	synaptics_rmi4_suspend_public(100);
#endif
		return 0;
	}
...
#ifdef DO_STARTUP_FW_UPDATE
	cancel_work_sync(&fwu_2nd->fwu_work);
	flush_workqueue(fwu_2nd->fwu_workqueue);
	destroy_workqueue(fwu_2nd->fwu_workqueue);
#endif
...
exit:
	complete(&fwu_remove_complete_2nd);
...
static int __init rmi4_fw_update_module_init(void)
{
	...
	synaptics_rmi4_new_function_2nd(&fwu_module, true);
...
{
	synaptics_rmi4_new_function_2nd(&fwu_module, false);

	wait_for_completion(&fwu_remove_complete_2nd);
