...
* and if it is and necessary, we're enable/disable fingerprint irq as well ...
*/
				...
				unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && evdata->info && fpc1020) {
		blank = evdata->data;
		pr_info("fpc1020 %s node:%d blank:%d (4->powerdown, 0-> unblank)\n",
			__func__, evdata->info->node, *blank);
	...
	pr_info("fpc1020 %s fb0_on_state:%d fb1_on_state:%d irq_enabled:%d\n",
		__func__, fb0_on_state, fb1_on_state, fpc1020->irq_enabled);
	if (((fb0_on_state == true) || (fb1_on_state == true)) && (fpc1020->irq_enabled == false)) {
		enable_irq(gpio_to_irq(fpc1020->irq_gpio));
		fpc1020->irq_enabled = true;
		pr_info("fpc1020 enable_irq %s blank:%d\n", __func__, *blank);
	} else if (((fb0_on_state == false) && (fb1_on_state == false)) && (fpc1020->irq_enabled == true)) {
		disable_irq(gpio_to_irq(fpc1020->irq_gpio));
		fpc1020->irq_enabled = false;
		pr_info("fpc1020 disable_irq %s blank:%d\n", __func__, *blank);
	}

	return 0;
}

static int fpc1020_register_fb(struct fpc1020_data *fpcd)
{
	memset(&fpcd->fb_notif, 0, sizeof(fpcd->fb_notif));
	fpcd->fb_notif.notifier_call = fb_notifier_callback;

	return fb_register_client(&fpcd->fb_notif);
}
#endif
#endif
