	...
	if (!(buffer->flags & ION_FLAG_POOL_FORCE_ALLOC)) {
		...
		if (zte_ion_sys_dedi_mem_config()) {
			if (page) {
				mutex_lock(&ion_sys_mutex);
				/* max stats update */
				zte_ion_sys_dm_cur_count_dec(tmp_index,
					cached);
				mutex_unlock(&ion_sys_mutex);
			}
		...
		} else { /* do as the original process */
			if (buffer->private_flags &
					...
					int nr_to_scan)
{
	...
	for (i = 0; i < num_orders; i++) {
		...
		if (zte_ion_sys_dedi_mem_config())
			nr_freed += ion_page_pool_shrink(pool,
				gfp_mask,
				nr_to_scan,
				zte_ion_dedi_get_order_count(i, 0));
		else
			nr_freed += ion_page_pool_shrink(pool,
				gfp_mask,
				nr_to_scan,
				0); /* don't reserve as original using */
#else
		...
		if (zte_ion_sys_dedi_mem_config())
			nr_freed += ion_page_pool_shrink(pool,
				gfp_mask,
				nr_to_scan,
				zte_ion_dedi_get_order_count(i, 1));
		else
			nr_freed += ion_page_pool_shrink(pool,
				gfp_mask,
				nr_to_scan,
	...
	if (!only_scan && zte_ion_sys_debug_shrink())
		pr_info("zte_ion_dbg shrink total=%d\n", nr_total);
#endif
