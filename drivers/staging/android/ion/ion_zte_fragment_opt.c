...
static const unsigned int order_to_k[ORDER_USED_NUM] = {4,
	64, 1024, 2048};
static const unsigned int order_index_to_order[ORDER_USED_NUM] = {
	9, 8, 4, 0};
static const unsigned int order_index_to_pages[ORDER_USED_NUM] = {
	512, 256, 16, 1};
...
static int zte_ion_sys_dedi_yellow_value_set(int new_yellow_value)
{
	...
	/* 120 loops (about 60M+, may none in some loops) */
	int guard_loop = 120; /* just to avoid possible while(1) */
...
/**
 * The shrink need to know the limit of a order count.
 * It should be used after the dedi config judgement.
 * The dedi count usually fixed after system startup.
 * @param index: which order to be filtered.
 * @param cached: which order pool.
 * @return: the dedi order count limit, not pages.
 */
int zte_ion_dedi_get_order_count(int index, int cached)
{
	int order_count = 0; /* default is the ignore value */
